...
const healthcheck = require("topcoder-healthcheck-dropin"); // eslint-disable-line @typescript-eslint/no-require-imports
  ...
  for (const m of messageSet) {
    try {
      const messageValue = m.message.value
        ? m.message.value.toString("utf8") // eslint-disable-line @typescript-eslint/no-unsafe-call
        : null;
      logger.info(
        `Handle Kafka event message; Topic: ${topic}; Partition: ${partition}; Offset: ${m.offset}; Message: ${messageValue}.`,
      );
...
const check = (): boolean => {
  ...
  consumerClient.initialBrokers.forEach((conn: any) => {
    logger.debug(`url ${conn.server()} - connected=${conn.connected}`); // eslint-disable-line @typescript-eslint/no-unsafe-call
...
const topics: string[] = [
  config.get("CHALLENGE_NOTIFICATION_EVENTS_TOPIC"),
  config.get("SUBMISSION_NOTIFICATION_AGGREGATE_TOPIC"),
  config.get("NOTIFICATION_AUTOPILOT_EVENTS_TOPIC"),
];
...
async function start() {
  ...
  healthcheck.init([check]); // eslint-disable-line @typescript-eslint/no-unsafe-call
