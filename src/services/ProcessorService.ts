...
const apiClient = axios.create({
  ...
  headers: {
    ...
    Authorization: `Bearer ${config.get("M2M_TOKEN")}`, // eslint-disable-line @typescript-eslint/restrict-template-expressions
  },
  timeout: 30000,
});
...
async function processRegistration(
  ...
  try {
    ...
    await PrismaService.createRegistration(userId, challenge.id, 0);
    logger.info(
      `Completed processing registration for user ${userId} to challenge ${challengeId}.`,
    );
  } catch (error: any) {
    // This catch block will handle potential foreign key constraint errors if the user does not exist in the User table.
    logger.error(
      `Failed to create registration for user ${userId} in challenge ${challengeId}. The user may not exist or another error occurred.`,
    );
...
async function processReview(payload: ReviewPayload): Promise<void> {
  logger.info("Process review event.");
  ...
  const submitTime = new Date(
    momentTZ.tz(submission.created, timeZone).format(),
  );
  ...
  logger.info(
    `Completed processing review for submission ${payload.submissionId}.`,
  );
}

async function processReviewSummation(
  payload: ReviewSummationPayload,
): Promise<void> {
  logger.info("Process review summation event.");
  const submission = await getSubmission(payload.submissionId);
  const { challengeId, memberId: userId } = submission;
  ...
  logger.info(
    `Completed processing review summation for submission ${payload.submissionId}.`,
  );
}

async function processReviewEnd(
  payload: AutopilotReviewEndPayload,
): Promise<void> {
  if (payload.phaseTypeName !== "Review" || payload.state !== "End") {
    logger.info(
      `Ignoring event for project ${payload.projectId}, not a Review End event.`,
    );
    return;
  }

  logger.info(`Processing Review End event for project ${payload.projectId}`);
  const challenge: any = await PrismaService.getChallengeByLegacyId(
    payload.projectId,
  );
  if (!challenge) {
    logger.info(
      `Challenge with legacy ID: ${payload.projectId} not found, ignoring event.`,
    );
    return;
  }

  // find challenge type base on track and subTrack
  const track = _.get(challenge, "legacyRecord.track", "DEVELOP");
  const subTrack = _.get(challenge, "legacyRecord.subTrack", "CODE");
  logger.info(`Challenge track is: ${track}, subTrack is: ${subTrack}`);
  ...
  try {
    await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
      ...
      if (results.length === 0) {
        logger.info(
          `No results found for challenge ${challenge.id}, skipping rating updates.`,
        );
        return;
      }

      results.sort(
        (a, b) => (b.system_point_total ?? 0) - (a.system_point_total ?? 0),
      );
      ...
      for (let i = 0; i < results.length; i++) {
        ...
        const placement =
          i > 0 &&
          result.system_point_total === results[i - 1].system_point_total
            ? results[i - 1].placed
            : i + 1;

        const { rating, volatility } = await MockRatingService.getRatingAndVol(
    ...
    logger.info(
      `Completed processing Review End event for project ${payload.projectId}.`,
    );
  } catch (error: any) {
    logger.error(
      `Transaction failed during Review End processing for project ${payload.projectId}: ${error.message}`,
    );
  }
}
...
async function processMessage(parsedMessage: any): Promise<void> {
  ...
  try {
    if (topic === config.get("CHALLENGE_NOTIFICATION_EVENTS_TOPIC")) {
      ...
      topic === config.get("SUBMISSION_NOTIFICATION_AGGREGATE_TOPIC")
    ) {
      if (payload.resource === constants.resources.review) {
        ...
        logger.info(
          "Ignoring event, resource doesn't match review or reviewSummation.",
        );
      }
    ...
    if (error instanceof z.ZodError) {
      logger.error(
        `Validation failed for message on topic ${topic}: ${JSON.stringify(error.errors)}`,
      );
    } else if (error instanceof Error) {
      logger.error(
        `An unexpected error occurred during processing: ${error.message}`,
      );
    } else {
      logger.error(
        "An unexpected and unknown error occurred during processing.",
      );
    }
  }
}
